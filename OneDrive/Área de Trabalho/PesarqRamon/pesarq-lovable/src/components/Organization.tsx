...
import { OrganizationInfoModal } from "./organization/OrganizationInfoModal";
import { ArchiveSectorModal } from "./organization/ArchiveSectorModal";
import { SectorModal } from "./organization/SectorModal";
import { SectorImportModal } from "./organization/SectorImportModal";
import { TeamMemberModal } from "./organization/TeamMemberModal";
import { ArchivalFundModal } from "./organization/ArchivalFundModal";
import { ArchivalFundViewModal } from "./organization/ArchivalFundViewModal";
import { AuthorityModal } from "./organization/AuthorityModal";
import { AuthorityViewModal } from "./organization/AuthorityViewModal";
...
export default function Organization() {
  ...
  const {
    ...
    deleteSector,
    importSectors
  } = useOrganizationSectors(organizationId || '');
  ...
  const {
    ...
    updateFund,
    deleteFund
  } = useArchivalFunds(organizationId || '');
  ...
  const {
    ...
    updateAuthority,
    deleteAuthority
  } = useAuthorities(organizationId || '');

  // Hook para dados específicos do setor de arquivo
  const {
    fetchArchiveSectorData,
    createArchiveSectorData,
    updateArchiveSectorData,
    loading: archiveSectorLoading
  } = useArchiveSectorData(organizationId);
  ...
  }, [organizationId, fetchArchiveSectorData]);
  ...
  // Handler para salvar informações da organização no banco
  ...
  // Handler para salvar dados do setor de arquivo na nova tabela
  const handleSaveArchiveSector = async (data: ArchiveSectorData): Promise<void> => {
    if (!organizationId) {
      ...
      throw new Error('ID da organização não encontrada');
    }
  ...
  // Handlers para fundos - CORRIGIDOS com melhor tratamento de erro
  ...
  const handleUpdateFund = async (fundId: string, fundData: any) => {
    console.log('📝 Updating fund:', fundId, 'with data:', fundData);
    
    try {
      const result = await updateFund(fundId, fundData);
      console.log('✅ Fund updated successfully:', result);
      return result;
    } catch (error) {
  ...
  const organizationData = {
    ...
    onCreateMember: () => setTeamMemberModal({
      isOpen: true,
      editingMember: null
    }),
    onEditMember: (member: any) => setTeamMemberModal({
      isOpen: true,
  ...
  return (
    <div className="space-y-4 sm:space-y-6 p-2 sm:p-4">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3">
        <div>
          ...
          <p className="text-gray-600 mt-1 text-sm sm:text-base">
            Gestão de informações organizacionais e arquivo
          </p>
        </div>
      </div>
      ...
      <TeamMemberModal 
        ...
        sectors={sectors} 
        onSectorCreated={() => {
          // Recarregar setores quando um novo for criado
          console.log('Novo setor criado, recarregando lista...');
        }} 
      />
      ...
      <ArchivalFundModal 
        ...
        onSave={async (data) => {
          if (editingFund) {
            await handleUpdateFund(editingFund.id, data);
          } else {
            await handleCreateFund(data);
          }
          setArchivalFundModalOpen(false);
          setEditingFund(null);
        }} 
